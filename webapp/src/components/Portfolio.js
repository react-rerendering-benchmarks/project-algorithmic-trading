...
const sellStock = (stock, qty, price) => {
  alert("Stock Sold!");
  fetch(`${API_ENDPOINT}/tradester/sell_stock/?stock=${stock}&quantity=${qty}&price=${price}`, {
    headers: {
...
const genData = (name, data) => {
  let color = "";
  while (color.length < 7) color = "#" + Math.floor(Math.random() * 16777215).toString(16);
  let setData = [{
    name: name,
    data: data,
    color: color
...
const loadPortfolio = id => fetch(`${API_ENDPOINT}/tradester/display_portfolio/?user_id=${id}`, {
  headers: {
    "Content-Type": "application/json",
    authorization: `Bearer ${localStorage.getItem("access_token")}`
  }
}).then(response => response.ok ? response : Promise.reject(response)).then(response => response.json());
      ...
      if (data) {
        ...
        for (let i = 1; i < datapoints.length - 2; i++) {
          ...
          for (let close_value of data[datapoints[i]].close_values) {
            ...
            close_dates.push(date.toLocaleDateString('en-US', {
              timeZone: 'UTC',
              month: 'short',
              day: '2-digit',
              year: 'numeric'
         ...
         * profit
         */
        return <div className="stockContainer">
              <h1>{username}'s Portfolio</h1>
              <h1>Balance: ${balance}</h1>

              {stocks.map(stock => <div className="stockDisplay">
                  <h2>Stock: [ {stock[0]} ]</h2>
                  <h3>Owned: {stock[3]}</h3>
                  <Chart options={state.options} series={genData(stock[0], stock[6])} type="line" height={350} />
                  <h3>Profits: ${stock[5]}</h3>
                  {username === localStorage.getItem('username') && <button className="sellbtn" onClick={() => sellStock(stock[0], stock[3], stock[4])}>
                      Sell
                    </button>}
                </div>)}
            </div>;
